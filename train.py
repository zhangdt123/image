...
class AdvancedContrastiveModel(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        # 初始化队列时使用更明确的尺寸
        self.register_buffer("queue", torch.zeros(Config.proj_hidden_dims[-1], Config.queue_size))
        self.queue = F.normalize(self.queue, dim=0)
        self.queue_ptr = 0  # 改为整数类型存储指针
        # 骨干网络
    ...
    @torch.no_grad()
    def enqueue_dequeue(self, keys):
        keys = keys.detach().clone()  # 防止引用问题
        batch_size = keys.size(0)
        feature_dim = keys.size(1)

        # 尺寸强制校验
        assert self.queue.size(0) == feature_dim, f"特征维度不一致：队列{self.queue.size(0)} vs 输入{feature_dim}"
        assert self.queue_ptr < Config.queue_size, f"队列指针异常：{self.queue_ptr}"

        # 环形更新实现
        start_idx = self.queue_ptr
        end_idx = self.queue_ptr + batch_size

        if end_idx <= Config.queue_size:
            # 直接替换
            self.queue[:, start_idx:end_idx] = keys.T
        else:
            # 分两部分替换
            wrap_size = Config.queue_size - start_idx
            # Part1: 填充到队列末尾
            self.queue[:, start_idx:] = keys[:wrap_size].T
            # Part2: 填充到队列开头
            self.queue[:, :batch_size - wrap_size] = keys[wrap_size:].T

        # 更新指针（自动循环）
        self.queue_ptr = end_idx % Config.queue_size
...
class HybridContrastiveLoss(nn.Module):
    ...
    def forward(self, online_emb, mom_emb, queue, temp):
        # 在损失计算时确保使用队列的前N列（N是合法尺寸）
        valid_size = Config.queue_size - (Config.queue_size % online_emb.size(0))
        valid_queue = queue[:, :valid_size]

        # 调整后损失计算
        pos_sim = torch.einsum('nd,nd->n', online_emb, mom_emb)
        neg_sim = torch.einsum('nd,dk->nk', online_emb, valid_queue)
...
def main():
    ...
    for epoch in range(Config.epochs):
        ...
        if avg_loss<best_loss:
            best_loss = avg_loss
            torch.save(model.state_dict(), f"model_epoch{2}.pth")

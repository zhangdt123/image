...
class DynamicProjection(nn.Module):
    def __init__(self, in_dim=768, hidden_dims=[1024, 512, 256], use_bn=True):
        super().__init__()
        layers = []
...
class AdvancedContrastiveModel(nn.Module):
    def __init__(self):
        ...
        self.projector = DynamicProjection(768, Config.proj_hidden_dims)
    ...
    def _build_enhanced_backbone(self):
        base = torchvision.models.vit_b_16(pretrained=True)
        # 移除分类头
        class FeatureExtractor(nn.Module):
            def __init__(self, model):
                ...
                self.encoder_pos_embedding = model.encoder.pos_embedding

            def forward(self, x):
                # 参考 torchvision.models.vit_b_16 的前向传播
                x = self.conv_proj(x)  # [batch_size, hidden_dim, 14, 14]
                x = x.reshape(x.size(0), x.size(1), -1).permute(0, 2, 1)  # [batch_size, seq_length, hidden_dim]
                batch_size = x.size(0)
                cls_token = self.class_token.expand(batch_size, -1, -1)  # [batch_size, 1, hidden_dim]
                x = torch.cat([cls_token, x], dim=1)  # [batch_size, seq_length + 1, hidden_dim]
                x = x + self.encoder_pos_embedding
                x = self.encoder(x)  # [batch_size, seq_length + 1, hidden_dim]
                return x

        return FeatureExtractor(base)
    ...
    def forward(self, x):
        assert x.dim() == 4, f"Expected input shape [batch_size, 3, 224, 224], got {x.shape}"
        online_feat = self.backbone(x)  # [batch_size, seq_length, hidden_dim]
        assert online_feat.dim() == 3, f"Expected [batch_size, seq_length, hidden_dim], got {online_feat.shape}"
        online_feat = online_feat[:, 0]  # [batch_size, hidden_dim]
        assert online_feat.shape[-1] == 768, f"Expected [batch_size, 768], got {online_feat.shape}"
        online_emb, temp = self.projector(online_feat)
        online_emb = F.normalize(online_emb, dim=1)

        with torch.no_grad():
            mom_feat = self.momentum_encoder[0](x)
            mom_feat = mom_feat[:, 0]
            mom_emb, _ = self.momentum_encoder[1](mom_feat)
    ...
    for epoch in range(Config.epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch + 1}/{Config.epochs}"):
            ...
            with torch.no_grad():
                ...
                neg_emb = model.momentum_encoder[1](neg_emb[:, 0])[0].detach()
        ...
        if avg_loss < best_loss:
            best_loss = avg_loss
            torch.save(model.state_dict(), f"model_epoch{1}_vit_b_16.pth")

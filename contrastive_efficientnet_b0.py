...
class DynamicProjection(nn.Module):
    ...
    def __init__(self, in_dim=1280, hidden_dims=[1024, 512, 256], use_bn=True):
        super().__init__()
        layers = []
...
class AdvancedContrastiveModel(nn.Module):
    ...
    def __init__(self):
        ...
        # 动态投影头
        self.projector = DynamicProjection(1280, Config.proj_hidden_dims)
    ...
    def _build_enhanced_backbone(self):
        ...
        return nn.Sequential(*list(base.children())[:-1])  # 移除分类头
...
def main():
    ...
    for epoch in range(Config.epochs):
        ...
        if avg_loss < best_loss:
            best_loss = avg_loss
            torch.save(model.state_dict(), f"model_epoch_effi{1}.pth")
